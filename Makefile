...
PHP_VERSION := 8.2
IMAGE_NAME := mezzio/problem-details
...
bump-unused:
	@$(call MK_INFO,"Bumping composer-unused")
	@docker run $(DOCKER_PHP) composer update -q
	@docker run $(DOCKER_PHP) composer bump -Dq
	@docker run $(DOCKER_PHP) composer update -q
.PHONY: bump-unused
...
composer-require-checker: install-crc ## Check for symbols from un-declared dependencies
...
rector-fix: install-rector ## Apply Rector changes
